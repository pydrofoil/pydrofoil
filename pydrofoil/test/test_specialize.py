...
def test_specialize_union_if_only_single_constructor_is_used():
    from pydrofoil.test.test_ir import compare
    ztuplez3z5bv_z5unit = Struct('ztuplez3z5bv_z5unit', ('ztuplez3z5bv_z5unit0', 'ztuplez3z5bv_z5unit1'), (GenericBitVector(), Unit()), True)
    ztuplez3z5unionz0zzPTW_Error_z5unit = Struct('ztuplez3z5unionz0zzPTW_Error_z5unit', ('ztuplez3z5unionz0zzPTW_Error_z5unit0', 'ztuplez3z5unionz0zzPTW_Error_z5unit1'), (Union('zPTW_Error', ('zPTW_Access', 'zPTW_Ext_Error', 'zPTW_Invalid_Addr', 'zPTW_Invalid_PTE', 'zPTW_Misaligned', 'zPTW_No_Permission', 'zPTW_PTE_Update'), (Unit(), Unit(), Unit(), Unit(), Unit(), Unit(), Unit())), Unit()), True)
    zPTW_Error = Union('zPTW_Error', ('zPTW_Access', 'zPTW_Ext_Error', 'zPTW_Invalid_Addr', 'zPTW_Invalid_PTE', 'zPTW_Misaligned', 'zPTW_No_Permission', 'zPTW_PTE_Update'), (Unit(), Unit(), Unit(), Unit(), Unit(), Unit(), Unit()))
    ...
    i11 = block1.emit(FieldWrite, 'ztuplez3z5bv_z5unit1', [i9, UnitConstant.UNIT], Unit(), None, None)
    i12 = block1.emit(Operation, 'zTR_AddresszIUExceptionTypezIzKzCbzK', [i9], zTR_ResultzIUExceptionTypezIzKzCbzK, '`37 417:20-417:51', 'zz438')
    block1.next = Return(i12)
    graph = Graph('ztranslateAddr', [zvAddr], block1)
    graph.check()

    codegen = FakeCodeGen()
    spec = Specializer(graph, codegen)
    codegen.specialization_functions['ztranslateAddr'] = spec
    a, b = spec._make_stub(((SmallFixedBitVector(64), None), ))
    assert a
    assert b == SmallFixedBitVector(64)
    assert spec.union1_graph_to_variant[a] == (zTR_ResultzIUExceptionTypezIzKzCbzK, 'zTR_AddresszIUExceptionTypezIzKzCbzK')
    ...
    block18 = Block()
    i1 = block0.emit(GlobalRead, 'zPC', [], SmallFixedBitVector(64), None, None)
    i10 = block0.emit(Operation, 'ztranslateAddr', [i1], zTR_ResultzIUExceptionTypezIzKzCbzK, '`85 24:17-24:49', 'zz46')
    i11 = block0.emit(GlobalRead, 'have_exception', [], Bool(), None, None)
    block0.next = ConditionalGoto(i11, block3, block4, '`85 24:17-24:49')
    block3.next = Return(DefaultValue(zFetchResult), None)
    i12 = block4.emit(UnionVariantCheck, 'zTR_FailurezIUExceptionTypezIzKzCbzK', [i10], Bool(), '`85 25:8-25:24', None)
...
block2 = Block()
i1 = block0.emit(GlobalRead, 'zPC', [], SmallFixedBitVector(64), None, None)
i2 = block0.emit(Operation, 'ztranslateAddr_specialized_o__union1_tup1_bv64', [i1], SmallFixedBitVector(64), '`85 24:17-24:49', 'zz46')
i3 = block0.emit(GlobalRead, 'have_exception', [], Bool(), None, None)
block0.next = ConditionalGoto(i3, block1, block2, '`85 24:17-24:49')
block1.next = Return(DefaultValue(Union('zFetchResult', ('zF_Base', 'zF_Error', 'zF_Ext_Error', 'zF_RVC'), (SmallFixedBitVector(32), Struct('ztuplez3z5unionz0zzExceptionType_z5bv64', ('ztuplez3z5unionz0zzExceptionType_z5bv640', 'ztuplez3z5unionz0zzExceptionType_z5bv641'), (Union('zExceptionType', ('zE_Breakpoint', 'zE_Extension', 'zE_Fetch_Access_Fault', 'zE_Fetch_Addr_Align', 'zE_Fetch_Page_Fault', 'zE_Illegal_Instr', 'zE_Load_Access_Fault', 'zE_Load_Addr_Align', 'zE_Load_Page_Fault', 'zE_M_EnvCall', 'zE_Reserved_10', 'zE_Reserved_14', 'zE_SAMO_Access_Fault', 'zE_SAMO_Addr_Align', 'zE_SAMO_Page_Fault', 'zE_S_EnvCall', 'zE_U_EnvCall'), (Unit(), Unit(), Unit(), Unit(), Unit(), Unit(), Unit(), Unit(), Unit(), Unit(), Unit(), Unit(), Unit(), Unit(), Unit(), Unit(), Unit())), SmallFixedBitVector(64)), True), Unit(), SmallFixedBitVector(16)))), None)
