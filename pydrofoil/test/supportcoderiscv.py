...
class MmapMemory(MemBase):
    ...
    def __init__(self):
        mem = rmmap.c_mmap(
            NonConstant(rmmap.NULL),
            NonConstant(self.SIZE),
            NonConstant(rmmap.PROT_READ | rmmap.PROT_WRITE),
            NonConstant(rmmap.MAP_PRIVATE | rmmap.MAP_ANONYMOUS),
            NonConstant(-1), NonConstant(0))
        mem = rffi.cast(rffi.UNSIGNEDP, mem)
        self.mem = mem

    def close(self):
        rmmap.c_munmap_safe(rffi.cast(rffi.CCHARP, self.mem), NonConstant(self.SIZE))
