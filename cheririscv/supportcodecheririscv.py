...
if 1: #with open(os.path.join(os.path.dirname(__file__), "riscv_model_version")) as f:
    SAIL_RISCV_VERSION = "add me later"
...
def platform_read_mem(machine, read_kind, addr_size, addr, n):
    if n == 16:
        word1 = machine.g.mem.read(addr.touint() + 8, 8)
        word2 = machine.g.mem.read(addr.touint(), 8)
        return bitvector.SmallBitVector(64, word1).append_64(word2)
    else:
        assert n <= 8
        addr = addr.touint()
...
Run the Pydrofoil Cheri-RISC-V emulator on elf_file.
