...
outriscvpys = [
    os.path.join(thisdir, "generated/outriscv32.py"),
    os.path.join(thisdir, "generated/outriscv.py"),
]
...
def _make_code(rv64=True):
    ...
    with open(riscvirs[rv64], "rb") as f:
        s = f.read()
    support_code = "from cheririscv import supportcodecheririscv as supportcode"
    entrypoints = "ztick_clock ztick_platform zinit_model zstep zext_decode zencdec_forwards zencdec_compressed_backwards".split()
    ...
    if rv64:
        ...
        from cheririscv.generated import outriscv32 as outriscv
    return outriscv
...
def make_code(rv64=True):
    ...
    return supportcodecheririscv.get_main(outriscv, rv64)
...
def make_code_combined():
    ...
    machinecls64 = supportcodecheririscv.get_main(mod64, True)._machinecls
    mod32 = _make_code(False)
    machinecls32 = supportcodecheririscv.get_main(mod32, False)._machinecls

    def bound_main(argv):
        return supportcodecheririscv.main(argv, machinecls64, machinecls32)
