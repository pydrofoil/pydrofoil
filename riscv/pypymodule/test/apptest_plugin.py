...
def test_union_repr():
    ...
    assert (
        repr(ast)
        == "ADDIW(bitvector(12, 0x7FD), bitvector(5, 0b00011), bitvector(5, 0b00101))"
    )
...
def test_struct():
    ...
    tlb = _pydrofoil.RISCV64().types.TLB_Entry(
        bv64, bv, False, bv64, bv64, bv64, bv64, bv64, bv64
    )
    assert tlb.age == bv64
...
def test_compact_union():
    ...
    assert x[0] == "Read_RISCV_reserved_strong_acquire"
    assert x == x
    assert repr(x) == "Some<Eread_kind%>('Read_RISCV_reserved_strong_acquire')"
...
def test_sailfunction_type():
    ...
    assert isinstance(
        typ.arguments[0], _pydrofoil.sailtypes.SmallFixedBitVector
    )
...
def test_lowlevel_attribute_error():
    ...
    assert (
        str(info.value)
        == "'lowlevel' object has no attribute 'privLevel_to_bats'"
    )
...
def test_bound_lowlevel_repr():
    m = _pydrofoil.RISCV64()
    assert (
        repr(m.lowlevel.get_vlen_pow)
        == f"<bound sail function .lowlevel.get_vlen_pow of {m}>"
    )
...
def test_bitvector_shift():
    ...
    assert _pydrofoil.bitvector(5, 0b100).arithmetic_rshift(
        1
    ) == _pydrofoil.bitvector(5, 0b10)
...
def test_step_intercept_mem():
    ...
    callbacks = _pydrofoil.Callbacks(
        mem_read8_intercept=read, mem_write8_intercept=write
    )
...
def test_dont_do_reset_vector_without_elf():
    ...
    ram_base = 0x80000000
    cpu.write_memory(
        ram_base, _pydrofoil.bitvector(64, 0x02028593)
    )  # addi a1, t0, 0x20
    cpu.write_register("pc", ram_base)
    cpu.step()
...
def test_set_sail_memory_bounds():
    mem = {}
    ...
    cpu = _pydrofoil.RISCV64(callbacks=callbacks)
    ram_base = 0x8000000000  # outside of default mem bounds
    cpu._set_sail_memory_bounds(ram_base, 0x100000)
    cpu.write_memory(
        ram_base, _pydrofoil.bitvector(64, 0x02028593)
    )  # addi a1, t0, 0x20
    cpu.write_register("pc", ram_base)
...
def test_sailtype_repr():
    assert (
        repr(_pydrofoil.sailtypes.SmallFixedBitVector(12))
        == "_pydrofoil.sailtypes.SmallFixedBitVector(12)"
    )
    assert (
        repr(_pydrofoil.sailtypes.BigFixedBitVector(121))
        == "_pydrofoil.sailtypes.BigFixedBitVector(121)"
    )
    cpu = _pydrofoil.RISCV64()
    rs = dict(cpu.register_info())
    assert (
        repr(rs["cur_privilege"])
        == "<_pydrofoil.sailtypes.Enum Privilege { User Supervisor Machine }>"
    )
    assert (
        repr(_pydrofoil.sailtypes.MachineInt())
        == "_pydrofoil.sailtypes.MachineInt()"
    )
    assert (
        repr(_pydrofoil.sailtypes.GenericBitVector())
        == "_pydrofoil.sailtypes.GenericBitVector()"
    )
...
def test_fvec():
    ...
    assert (
        repr(fvectype)
        == "<_pydrofoil.sailtypes.FVec 32 _pydrofoil.sailtypes.SmallFixedBitVector(64)>"
    )
...
def test_union_repr():
    cpu = _pydrofoil.RISCV64()
    assert (
        repr(cpu.lowlevel.accessToFault.sail_type.arguments[0])
        == "<_pydrofoil.sailtypes.Union AccessType<u>>"
    )
...
def test_struct_repr():
    cpu = _pydrofoil.RISCV64()
    assert (
        repr(cpu.lowlevel.check_PTE_permission.sail_type.arguments[4])
        == "<_pydrofoil.sailtypes.Struct PTE_Flags>"
    )
...
def test_tuple_repr():
    cpu = _pydrofoil.RISCV64()
    assert (
        repr(dict(cpu.types.ast.sail_type.constructors)["ITYPE"])
        == "<_pydrofoil.sailtypes.Tuple (_pydrofoil.sailtypes.SmallFixedBitVector(12), _pydrofoil.sailtypes.SmallFixedBitVector(5), _pydrofoil.sailtypes.SmallFixedBitVector(5), <_pydrofoil.sailtypes.Enum iop { RISCV_ADDI RISCV_SLTI RISCV_SLTIU RISCV_XORI RISCV_ORI RISCV_ANDI }>)>"
    )
...
def test_function_repr():
    cpu = _pydrofoil.RISCV64()
    assert (
        repr(cpu.lowlevel.check_PTE_permission.sail_type)
        == "<_pydrofoil.sailtypes.Function (<_pydrofoil.sailtypes.Union AccessType<u>>, <_pydrofoil.sailtypes.Enum Privilege { User Supervisor Machine }>, _pydrofoil.sailtypes.Bool(), _pydrofoil.sailtypes.Bool(), <_pydrofoil.sailtypes.Struct PTE_Flags>, _pydrofoil.sailtypes.SmallFixedBitVector(64), _pydrofoil.sailtypes.Unit()) -> <_pydrofoil.sailtypes.Union PTE_Check>>"
    )
...
def test_fixedbitvector_base_class():
    assert isinstance(
        _pydrofoil.sailtypes.SmallFixedBitVector(12),
        _pydrofoil.sailtypes.FixedBitVector,
    )
    assert isinstance(
        _pydrofoil.sailtypes.BigFixedBitVector(122),
        _pydrofoil.sailtypes.FixedBitVector,
    )

    # but constructing a FixedBitVector gives one of the subclasses
    assert isinstance(
        _pydrofoil.sailtypes.FixedBitVector(12),
        _pydrofoil.sailtypes.SmallFixedBitVector,
    )
    assert isinstance(
        _pydrofoil.sailtypes.FixedBitVector(122),
        _pydrofoil.sailtypes.BigFixedBitVector,
    )
