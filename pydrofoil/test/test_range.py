...
ints = strategies.builds(
    ...
    special_values
    | strategies.integers(min_value=int(-sys.maxint - 1), max_value=sys.maxint)
    | strategies.integers(),
)
...
unbounded = strategies.builds(lambda x: (Range(None, None), int(x)), ints)

lower_bounded = strategies.builds(
    lambda x, y: (Range(min(x, y), None), max(x, y)), ints, ints
)

upper_bounded = strategies.builds(
    lambda x, y: (Range(None, max(x, y)), min(x, y)), ints, ints
)

bounded = strategies.builds(
    build_bound_with_contained_number, ints, ints, ints
)
...
def test_intersect_example():
    assert Range(None, None).intersect(Range(None, None)) == Range(None, None)
    assert Range(0, 10).intersect(Range(5, 15)) == Range(5, 10)
    assert Range(None, 10).intersect(Range(5, 15)) == Range(5, 10)
    assert Range(None, 10).intersect(Range(None, 15)) == Range(None, 10)
    assert Range(None, None).intersect(Range(5, 15)) == Range(5, 15)
    assert Range(0, None).intersect(Range(5, 15)) == Range(5, 15)
    assert Range(0, 10).intersect(Range(5, None)) == Range(5, 10)
    assert Range(0, 10).intersect(Range(None, 15)) == Range(0, 10)
    assert Range(0, 10).intersect(Range(15, 20)) == None
    assert Range(0, 10).intersect(Range(15, None)) == None
