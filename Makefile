...
## Cheri-RISC-V targets


.PHONY: regen-cheri-riscv-ir-files
regen-cheri-riscv-ir-files: isla/isla-sail/plugin.cmxs ## Regenerate the JIB IR files
	PATH=${realpath isla/isla-sail/}:${PATH} && export PATH && eval `opam config env --switch=sail/ --set-switch` && \
	cd sail-cheri-riscv && \
	isla-sail -O -Oconstant_fold -memo_z3 -c_include riscv_prelude.h -c_include riscv_platform.h -c_no_main \
		...
		sail-riscv/model/riscv_vmem_common.sail \
		sail-riscv/model/riscv_vmem_tlb.sail \
		sail-riscv/model/riscv_vmem_sv39.sail \
		sail-riscv/model/riscv_vmem_sv48.sail \
		sail-riscv/model/riscv_vmem_rv64.sail \
		sail-riscv/model/riscv_insts_begin.sail \
		sail-riscv/model/riscv_insts_base.sail \
