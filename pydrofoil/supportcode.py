...
@purefunction
def bitvector_concat_bv_gbv_truncate_to(
    machine, bv1, width, gbv, target_width
):
    return gbv.prepend_small_then_truncate_unwrapped_res(
        width, bv1, target_width
    )


@purefunction
def bitvector_concat_bv_bv_n_zeros_truncate(
    machine, bv1, width1, bv2, width2, nzeros, target_width
):
    res = (bv1 << width2) | bv2
    res <<= nzeros
...
@objectmodel.always_inline
@purefunction
def packed_field_cast_smallfixedbitvector(
    machine, targetwidth, (width, val, data)
):
    if not objectmodel.we_are_translated() and machine == "constfolding":
        from pydrofoil.ir import CantFold
    ...
    def c(space, w_val):
        if isinstance(w_val, bitvector.BitVector):
            if w_val.size() != width:
                raise oefmt(
                    space.w_ValueError,
                    "expected bitvector of size %d, got size %d",
                    width,
                    w_val.size(),
                )
...
def generate_convert_to_pypy_enum(cls, name):
    ...
    def c(space, val):
        try:
            ...
            raise oefmt(
                space.w_ValueError, "unknown value %d for enum %s", val, name
            )
...
def generate_convert_from_pypy_enum(cls, name):
    ...
    def c(space, w_val):
        try:
            ...
            raise oefmt(
                space.w_ValueError,
                "unknown enum value %R for enum %s",
                w_val,
                name,
            )
